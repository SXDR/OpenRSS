...
class MiT(nn.Module):
    def __init__(self, model_name: str = 'B0', channel: int = 3):
        super().__init__()
        ...
        # patch_embed
        self.patch_embed1 = PatchEmbed(channel, embed_dims[0], 7, 4)
...
class MiTrgb(nn.Module):
    ...
    def forward(self, x: Tensor, x1_t: Tensor, x2_t: Tensor, x3_t: Tensor, x4_t: Tensor) -> Tensor:
        B = x.shape[0]
        # stage 1
        x, H, W = self.patch_embed1(x)
        for blk in self.block1:
