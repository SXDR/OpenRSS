...
from util.FT_dataset import FT_dataset
...
if __name__ == '__main__':
    class_num = ["road", "ground_sidewalk", "building", "curb", "fence", "pole", "vegetation", "terrain", "sky",
                 "person", "car", "bicycle", "Background"]
    # class_num = ["building", "curb", "fence", "pole", "vegetation", "terrain", "sky",
    #              "person", "car", "bicycle", "Background"]
    text_list = []
    qz = "a pohot of "
    for i in class_num:
    ...
    test_dataset = FT_dataset(data_dir=args.data_dir, split=args.dataset_split, input_h=args.img_height,
    ...
    print(
        "* recall per class: \n    road: %.6f, ground_sidewalk: %.6f, building: %.6f, curb: %.6f, fence: %.6f, pole: %.6f, vegetation: %.6f, terrain: %.6f, sky: %.6f, person: %.6f, car: %.6f, bicycle: %.6f, unlabeled: %.6f" \
        % (recall_per_class[0], recall_per_class[1], recall_per_class[2], recall_per_class[3], recall_per_class[4],
           ...
           recall_per_class[10], recall_per_class[11], recall_per_class[12]))
    print(
        "* iou per class: \n    road: %.6f, ground_sidewalk: %.6f, building: %.6f, curb: %.6f, fence: %.6f, pole: %.6f, vegetation: %.6f, terrain: %.6f, sky: %.6f, person: %.6f, car: %.6f, bicycle: %.6f, unlabeled: %.6f" \
        % (iou_per_class[0], iou_per_class[1], iou_per_class[2], iou_per_class[3], iou_per_class[4],
           ...
           iou_per_class[10], iou_per_class[11], iou_per_class[12]))
